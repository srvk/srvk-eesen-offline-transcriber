...
graphdir=$1
data=$2
orig_dir=`echo $3 | sed 's:/$::g'` # remove any trailing slash.

if [ $# -eq 4 ]; then
    srcdir=$4; 
else
    srcdir=`dirname $orig_dir`; # assume model directory one level up from decoding directory.
fi
sdata=$data/split$nj;
...
## Set up the features
    ...
    $subsample_feats && feats="$feats subsample-feats --n=3 --offset=$offset ark:- ark:- |"
##
